...
//shader p dibujar el buffer de profundidad
Shader shaderDepth;
//shader p visualizar el buffer de profundidad(ayuda para el alumno)
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	/**** Inicializacion del frame buffer para almacenar la profundidad ******/
...
void applicationLoop() {
	...
	while (psi) {
		...
		//declarar variables para el buffer de profundidad
		// float near_plane=0.1f, far_plane=20.0;
		glm::mat4 lightProjection=glm::mat4(1.0),lightView=glm::mat4(1.0);
		glm::mat4 lightSpaceMatrix;
		// lightProjection=glm::ortho(-25.0f,25.0f,-25.0f,25.0f,near_plane,far_plane);//cambia si ampliamos el plano del piso
		// lightView=glm::lookAt(lightPos,glm::vec3(0.0f),glm::vec3(0.0f,1.0f,0.0f));
		lightProjection[0][0] = 1.0 / shadowBox -> getWidth();
		lightProjection[1][1] = 1.0 / shadowBox -> getHeight();
		lightProjection[2][2] = -2.0 / shadowBox -> getLength();
		lightProjection[3][2] = - (15.0 + 0.1) / shadowBox -> getLength();
		lightProjection[3][3] = 1.0f;
		lightView = glm::lookAt(centerBox, centerBox + glm::normalize(-lightPos), glm::vec3(0.0, 1.0, 0.0));
		lightSpaceMatrix=lightProjection*lightView;//render desde el pov de la luz
		...
		// Paso 1 - Render del buffer de profundidad desde el punto de vista de la luz

		glClearColor(0.1f,0.1f,0.1f,1.0f);
		//glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
		glViewport(0,0,SHADOW_WIDTH,SHADOW_HEIGHT);
		glBindFramebuffer(GL_FRAMEBUFFER,depthMapFBO);//frmae buffer, espacio de memoria, color y profundidad
		...
		/*//solo para debug, se comentara mas tarde
		glViewport(0,0,screenWidth,screenHeight);//config en la pantalla
		glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
