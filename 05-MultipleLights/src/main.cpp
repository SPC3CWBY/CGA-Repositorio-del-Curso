...
std::vector<float> lamp1Orientation = {
	-17.0, -82.67, 23.7 // Valores para los grados de giro
};
...
void applicationLoop() {
	...
	while (psi) {
		...
		for (int i = 0; i < lamp1Position.size(); i++){
			...
			modelLamp1.setOrientation(glm::vec3(0, lamp1Orientation[i], 0)); // Giro en el eje y
			modelLamp1.render();
		}
		...
		for (int i = 0; i < lamp2Position.size(); i++){
			...
			modelLamp2.setOrientation(glm::vec3(0, lamp2Orientation[i], 0)); // Giro en el eje y
			modelLamp2.render();

			// Lamp2 poste
			modelLamp2Post.setPosition(lamp2Position[i]);
			modelLamp2Post.setScale(glm::vec3(0.5));
			modelLamp2Post.setOrientation(glm::vec3(0, lamp2Orientation[i], 0)); // Giro en el eje y
