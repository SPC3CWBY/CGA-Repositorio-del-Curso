...
uniform sampler2D texture2; // Se agrega la posibilidad de la segunda textura
...
void main()
{
	...
	// Se asigna al Fragment color el mix de la textura 1 y 2
	// texture mix para la segunda textura del 30% = 0.3 
	FragColor = mix(texture(texture1, our_uv),texture(texture2, our_uv), 0.3);
	if(colorText.a < 0.1)
		discard;
